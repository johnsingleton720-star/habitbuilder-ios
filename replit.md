...
-   **Smart Plan Adjustment (Pro/Premium)**: When a user's completion rate drops below 40% after 5+ days, the app detects the plan isn't working and offers to regenerate it. The adjusted plan uses completion patterns, miss reasons, mood data (Pro: 3 entries, Premium: 7 entries), and journal themes (Premium only) to create a better-fitting plan. A Dashboard-level banner (`/api/habits/needs-adjustment`) also surfaces struggling habits prominently. The scheduler sends a daily push notification at noon for struggling habits. Suppression is server-side via `lastAdjustedAt` field on habits (7-day cooldown after adjust-plan or regenerate-plan), so the banner doesn't re-appear across devices after a plan is addressed.
...
### Mobile Applications
-   **iOS App**: Capacitor framework with iOS safe area insets (viewport-fit=cover, safe-top/safe-bottom CSS utilities), Keyboard/StatusBar/SplashScreen plugin config. Uses Apple In-App Purchase (`cordova-plugin-purchase` / CdvPurchase) for payments with server-side receipt validation (`/api/apple/validate-receipt`). Auth flow on native iOS uses a local `capacitor-auth-session` plugin (`plugins/capacitor-auth-session/`) wrapping `ASWebAuthenticationSession` with `prefersEphemeralWebBrowserSession = true` — this ensures no cached credentials from previous logins, enabling true account switching on shared devices. Falls back to `@capacitor/browser` on non-iOS native platforms. Token exchange bridge: `/api/auth/native-complete` → `habitbuilder://auth?token=xxx` → `/api/auth/exchange-token`. `@capacitor/app` handles deep link listening as fallback. `allowNavigation` only includes `habitbuilder.pro` (not `replit.com`), so OIDC navigates in the system browser. Build instructions in `ios-build-instructions.md` cover MacinCloud, Codemagic CI/CD, and direct Mac options. Codemagic Podfile is no longer overwritten; instead, the `post_install` fix is appended to the auto-generated Podfile to preserve plugin pods.
